...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    return (temperature < 800.00) and (neutrons_emitted > 500.00) and (temperature * neutrons_emitted) < 500000.00
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    """
    efficieny = ((voltage * current)/theoretical_max_power) * 100

    if efficieny < 30:
        return 'black'
    elif efficieny >= 30 and efficieny < 60:
        return 'red'
    elif efficieny >= 60 and efficieny < 80:
        return 'orange'
    else: 
        return 'green'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    """
    emission = temperature * neutrons_produced_per_second

    emission = temperature * neutrons_produced_per_second

    if emission < threshold * 0.9:
        return 'LOW'
    elif 0.9 * threshold <= emission <= 1.1 * threshold:
        return 'NORMAL'
    else:
        return 'DANGER'